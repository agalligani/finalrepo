...
//			content[c].appendTo(content_wrapper[c]);
			...
			var action_box = $('<div id="action_box">'+content[c]+content_embed[c]+'</div>');
//			content_wrapper[c].mouseup(function() {alert('heck yeah!');});
			
			
		content[c].bind('mouseover', function(e) {stop=1;});		
			content_embed[c].bind('mousedown', function(e) {alert('User clicked on "foo."');});		
			content_wrapper[c].bind('mousedown', function(e) {alert('User clicked on "foo."');});		
		...
		// TODO: add resource logic
			...
			var the_image = $(content_image[c]);
			 the_image.css({"height":"193px","width":"317px","cursor":"pointer"});
			 
			 the_image.click(function(){
						window.open(content_value[c], 'STLC Resource', "location=1,status=1,scrollbars=1");
						return false;
			  });
...
function add_overlay(v) {
		//create a background tint partially opaque
		
		var tinter = $('<div id="overlay_tint"></div>');
		$('body').append(tinter);
		tinter.css({'opacity':'0.4'});
		
		//create clear background to position the action box
		
		var clear_wrapper = $("<div id='overlay_wrapper'></div>");
		action_box.appendTo(clear_wrapper);
